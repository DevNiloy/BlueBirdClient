...
function initializeTranslateElement(container: HTMLElement): void {
  ...
  console.log(
    "[GoogleTranslate] Creating TranslateElement with VERTICAL layout...",
  );

  try {
    const InlineLayout = (window as any).google.translate.TranslateElement
      .InlineLayout;
  ...
  return new Promise((resolve, reject) => {
    ...
    const check = setInterval(() => {
      ...
      if (attemptCount % 20 === 0) {
        console.log(
          `[GoogleTranslate] Waiting for select... ${Math.floor(elapsed / 1000)}s`,
        );
      }
      ...
      if (selectElement) {
        console.log(
          "[GoogleTranslate] ✅ Select element found after",
          elapsed,
          "ms",
        );
        clearInterval(check);
    ...
    mutations.forEach((mutation) => {
      ...
      if (mutation.type === "childList") {
        mutation.addedNodes.forEach((node) => {
          if (
            node.nodeName === "FONT" &&
            (node as HTMLElement).className?.includes("translated")
          ) {
            console.log(
              "[Protection] Google Translate modified protected element",
            );
